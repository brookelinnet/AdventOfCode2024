...
'
IF OBJECT_ID('tempdb.dbo.#AdventDay3', 'U') IS NOT NULL
    DROP TABLE #AdventDay3;
CREATE TABLE #AdventDay3 (id int,
posbegin int,
posend int,
poslen int,
substr nvarchar(20),
...
xyprod int,
isenabled bit)
DECLARE @pos int=0, @posbegin int, @posend int, @x int, @y int, @xyprod int, @substr nvarchar(20)
WHILE @pos<len(@input)-11 BEGIN
    /* RESET Vals */
    SET @posbegin=0
    SET @posend=0
    SET @x=0
    ...
    SET @xyprod=0
    /* Find Starting Char */
    SET @posbegin=(SELECT CHARINDEX('mul(', @input, @pos))
    /* Find Ending Char */
    SET @posend=(SELECT CHARINDEX(')', @input, @posbegin))
    /* Retreive Substring from data input */
    SET @substr=(SELECT SUBSTRING(@input, @posbegin, (@posend-(@posbegin-1))))
    /* Pull X From Substring */
    IF((SELECT isnumeric((SELECT value
                          FROM STRING_SPLIT(LEFT(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4), len(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4))-1), ',')
                          ORDER BY value OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY)))=1 and(@posend-@posbegin<12))BEGIN
        SET @x=(SELECT value
                FROM STRING_SPLIT(LEFT(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4), len(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4))-1), ',')
                ORDER BY value OFFSET 0 ROWS FETCH NEXT 1 ROWS ONLY)
    END;
    /* Pull Y From Substring */
    IF((SELECT isnumeric((SELECT value
                          FROM STRING_SPLIT(LEFT(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4), len(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4))-1), ',')
                          ORDER BY value OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY)))=1 and(@posend-@posbegin<12))BEGIN
        SET @Y=(SELECT value
                FROM STRING_SPLIT(LEFT(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4), len(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4))-1), ',')
                ORDER BY value OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY)
    END;
    --SELECT @x as 'X', @y AS 'Y'
    /* Insert into TempTable */
    IF((@posend-@posbegin<12)AND(@x<>0)AND(@y<>0))
        INSERT INTO #AdventDay3(posbegin, posend, poslen, substr, x, y, xyprod)
        VALUES(@posbegin, @posend, @posend-@posbegin, (SELECT LEFT(RIGHT(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4), len(RIGHT(SUBSTRING(@input, 36, (@posend-@posbegin+1)), len(SUBSTRING(@input, @posbegin, (@posend-@posbegin+1)))-4))-1)), @x, @y, @x * @y)

    /* Set loop increment to end of last scanned substring */
    IF(@posend>1)BEGIN
        IF(@posend-@posbegin>=12)BEGIN
            SET @pos=@posbegin+1
        END ELSE IF(@posend-@posbegin<12)BEGIN
                     SET @pos=@posend
        END
    END;
    ELSE BEGIN
    ...
    END;
END;
IF OBJECT_ID('tempdb.dbo.#AdventDay3_Do', 'U') IS NOT NULL
    DROP TABLE #AdventDay3_Do;
CREATE TABLE #AdventDay3_Do (doid int,
position int,
do bit)
DECLARE @DoPosition INT=1
DECLARE @DoID INT=1
WHILE @DoPosition<len(@input)BEGIN
    /* Find Next Do */
    SET @DoPosition=(SELECT CHARINDEX('do()', @input, @DoPosition))
    SET @DoPosition=@DoPosition+1
    INSERT INTO #AdventDay3_Do(doid, position, do)
    VALUES(@doid, @DoPosition, 1)
    SET @DoID=@DoID+1
    IF((SELECT CHARINDEX('do()', @input, @DoPosition))=0)BEGIN
        SET @DoPosition=len(@input)
    END
END;
SET @DoPosition=1
WHILE @DoPosition<len(@input)BEGIN
    /* Find Next Do */
    SET @DoPosition=(SELECT CHARINDEX('don''t()', @input, @DoPosition))
    SET @DoPosition=@DoPosition+1
    INSERT INTO #AdventDay3_Do(doid, position, do)
    VALUES(@doid, @DoPosition, 0)
    SET @DoID=@DoID+1
    IF((SELECT CHARINDEX('don''t()', @input, @DoPosition))=0)BEGIN
        SET @DoPosition=len(@input)
    END
END;
...
DECLARE CheckDo CURSOR FOR SELECT posbegin FROM #AdventDay3
OPEN CheckDo
FETCH NEXT FROM CheckDo
INTO @CheckDoStart
WHILE @@FETCH_STATUS=0 BEGIN

    /*Find lowest available range*/
    SET @Position=(SELECT TOP 1 position
                   from #AdventDay3_Do
                   WHERE position<@CheckDoStart
                   order by position desc)
    SET @IsEnable=(SELECT TOP 1 do
                   from #AdventDay3_Do
                   WHERE position<@CheckDoStart
                   order by position desc)
    --PRINT @CheckDoStart
...
CLOSE CheckDo
DEALLOCATE CheckDo
SELECT (SELECT COUNT(*)FROM #AdventDay3_Do WHERE DO=1) AS 'DO', (SELECT COUNT(*)FROM #AdventDay3_Do WHERE DO=0) AS 'DON''T'
SELECT (SELECT SUM(xyprod)from #AdventDay3) AS 'All', (SELECT SUM(xyprod)from #AdventDay3 WHERE isenabled=1) AS 'All Enabled'
